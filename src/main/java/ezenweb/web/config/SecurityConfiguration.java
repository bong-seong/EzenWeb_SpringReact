...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
        ...
        http
                ...
                .authorizeHttpRequests() // 인증 요청
                    ...
                    .antMatchers("/admin/**") // localhost:8080/admin/** 이하 페이지는 모두 제한
                        .hasRole("admin")
                    // .antMatchers("/board/write") // 게시판 쓰기 페이지는 회원만 가능
                    //    .hasRole("user")
    ...
    // import org.springframework.web.cors.CorsConfigurationSource
    // 스프링 시큐리티에 CORS 정책 설정 [ 리액트의 요청을 받기 위해서 ]
    @Bean
    CorsConfigurationSource corsConfigurationSource(){
        CorsConfiguration corsConfiguration = new CorsConfiguration();
        corsConfiguration.setAllowedOrigins(Arrays.asList("http://localhost:3000")); // 주소
        corsConfiguration.setAllowedMethods(Arrays.asList("HEAD" , "GET" , "POST" , "PUT" , "DELETE") ); // http 메소드
